...
def test_web_servers():
	with open('./webserver_ips.data.test.res', 'r') as filehandle:	
		...
		for line in filecontents:
			...
			dst_port = 44345
			single_ip_forecast(ip, protocol, port, ns, dst_ip, dst_port, True, dataset)
...
def check_fn_ratelimit():
	ips = list()
	with open('../ipid_prediction/Dataset/online_analysis/web_servers.ratelimit.data') as filehandle:
			filecontents = filehandle.readlines()
			for line in filecontents:
				fields = line.split(",")
	...
	with open('../ipid_prediction/evaluate/online_analysis/lr.fast_scan.p80.fn.res') as filehandle:
			filecontents = filehandle.readlines()
			for line in filecontents:
				fields = line.split(",")
				if len(fields) < 1 : continue
...
def analysis():
	'''c = 0
	with open('../ipid_prediction/evaluate/online_analysis/ipid_port_scan.lr.web_servers.p44345.log') as filehandle:
			filecontents = filehandle.readlines()
			for line in filecontents:
				fields = line.split("|")
				if len(fields) > 1 : continue
	...
	f = open('../ipid_prediction/Dataset/online_analysis/reflectors.ratelimit.data', 'w')
	with open('../ipid_prediction/evaluate/online_analysis/reflectors_global.data.res.res') as filehandle:
			filecontents = filehandle.readlines()
			for line in filecontents:
				fields = line.split(",")
				if len(fields) < 2 : continue
	...
	ips = list()
	res1 = {}
	f = open('../ipid_prediction/evaluate/online_analysis/lr.fast_scan.p80.fn.ratelimit.log', 'w')
	with open('../ipid_prediction/evaluate/online_analysis/lr.fast_scan.p80.fn.ratelimit.res') as filehandle:
			filecontents = filehandle.readlines()
			for i, line in enumerate(filecontents):
				fields = line.split(",")
				if len(fields) < 1 : continue
	...
	f.close()
	res0 = {}
	with open('../ipid_prediction/evaluate/online_analysis/lr.fast_scan.p80.fn.ratelimit.log') as filehandle:
			filecontents = filehandle.readlines()
			for i, line in enumerate(filecontents):
				fields = line.split(",")
				if len(fields) < 1 : continue
