...
def group_ips_measure(ips, protocol, port, cus, pfxs, domains, dst_ip, dst_port, td, l, spoof, dataset):
	for ip, cu, pfx, ns in zip(ips, cus, pfxs, domains):
		controlled_experiment(ip, protocol, port, cu, pfx, ns, dst_ip, dst_port, td, l, spoof, dataset)
...
def group_ips_measure_old(ip, cu, pfx):
	...
	spoof = False
	print(ip, cu, pfx)
	controlled_experiment(ip, protocol, port, cu, pfx, '', dst_ip, dst_port, td, l, spoof, None) # ns = ''
...
def compute_td(sip, dst_ip, dst_port):
	...
	try:
		...
		run(sip, dst_ip, dst_port, pcapFile)
...
def single_ipid_test_for_spoof(cu, asn, ip, protocol, port, ns, plth, spoof, dataset, ofile):
	code = 0		
	sliding_window = list()
	pr = 1
	wlth = 5
	...
	while True:
		...
		if len(predictions) == plth-1:
			...
			if spoof:
				...
				g = ip.split(".")
				if len(g) < 4: continue
				dst_ip = g[0] +'.'+ g[1]+'.'+g[2]+'.'+str((int(g[3])+1)%256)
				dst_port = port
				spoofing_probe(dst_ip, protocol, dst_port, ns, ip, port, n, flag) #test_pred_n, port should be random
...
def single_ipid_test_for_censor(cu, asn, ip, protocol, port, ns, cls, dst_ip, dst_port, plth, spoof, td, dataset, ofile):
	code = 0		
...
def filter_unreachable_webservers():
	...
	with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
		...
		for label, url, dst_ip, dst_port in zip(labels, urls, ips, ports):
			futures.append(executor.submit(test_dst_port, label, url, dst_ip, 'tcp', 'S', dst_port, ''))
		for future in concurrent.futures.as_completed(futures):
...
def test_reflectors():
	...
	with open('./ipid_reflectors_EU.dat', 'r') as filehandle: # ./lr.reflectors.(low).res, scan_target_reflectors.res
		filecontents = filehandle.readlines()
		for line in filecontents:
			fields = line.split(",")
			...
			if len(ips) == 300: #10			
				with concurrent.futures.ThreadPoolExecutor(max_workers=300) as executor:
					futures = []
					for ip, cu, pfx, ns in zip(ips, cus, pfxs, domains):
						futures.append(executor.submit(controlled_experiment, ip, protocol, port, cu, pfx, ns, dst_ip, dst_port, td, l, spoof, dataset))
					for future in concurrent.futures.as_completed(futures):
						#print('Done!')
						continue
				ips = list()
...
def test_reflectors_analysis():
	f = open('../censorship_measurement/clients_data/ipid_reflectors_All.refine02.dat', 'w')
	ips = list()
	#mips = list()
...
def test_webservers():
	...
	for dst_ip, dst_port in zip(webservers, dst_ports):
		...
		td = compute_td(dst_ip, dst_port)
		single_censor_measure(ip, protocol, port, ns, dst_ip, dst_port, td, 30, True, dataset)
...
f = open('./ipid_reflectors_EU_filter.dat', 'w')
def main():
	test_webservers()
