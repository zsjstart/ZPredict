...
IPID-TSF provides three key outputs: Mean Absolute Error (MAE), Root Mean Square Error (RMSE), and Symmetric Mean Absolute Percentage Error (SMAPE), which serve as indicators of the predictability of target IPIDs.
Noting that these three outputs inherently represent prediction errors of predictive models on target IPID time series. Smaller prediction errors suggest higher predictability of target IPIDs.

## Z-test
We assume that prediction errors generated by the predictive (GP) model follow a Gaussian distribution, as evidenced by previous studies, such as [2].
...
In scenarios involving idle port scan and SAV measurement, tools like Nmap [3] and SMap [4] detect global IPID increments resulting from spoofed packets at a single time point. In contrast, techniques for censorship assessment or related studies, such as Ensafi [5] and Augur [6], analyze at least two IPID increments, taking into account potential TCP retransmissions.
...
<p align="center">
  <img src="https://github.com/zsjstart/ZPredict/blob/main/images/port_scan_fpr_fnr_plot.pdf" width="3500" title="">
</p>
...
![Figure 2](images/censor_measure_accs_plot.pdf) and ![Figure 3](images/censor_measure_fpr_fnr_plot.pdf) illustrate the validation results using datasets associated with censor measurement, including accuracy values across three states: "No blocking", "Ingress blocking", and "Egress blocking", as well as the false positive rate (FPR) and false negative rate (FNR) values.
