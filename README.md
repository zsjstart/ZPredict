...
Given N (e.g., 30) IPID predictions for a counter, we obtain the prediction errors $E =\left \{ e_{n} \right \}, n \in [0, N-1]$, where $e_{n}=\hat{x}_{n}-x_{n}$, with $\hat{x}_{n}$ and $x_{n}$ representing the predicted and actual values at the ${n}_{th}$ second, respectively.
