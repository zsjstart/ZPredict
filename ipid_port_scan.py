...
def test_via_nmap(ip, protocol, dst_port, dst_ip, ns, dataset):
    #astatus = test_dst_port(dst_ip, protocol, 'S', dst_port, ns)
    try:
...
def start_measure(reflectors, webservers, dataset):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
        ...
        for ip, dst_ip in zip(reflectors, webservers):
            futures.append(executor.submit(single_port_scan, ip, protocol, port, ns, dst_ip, dst_port, 30, False, dataset))
            #futures.append(executor.submit(test_via_nmap, ip,
                                           protocol, dst_port, dst_ip, ns, dataset))
        for future in concurrent.futures.as_completed(futures):
            future.result()
