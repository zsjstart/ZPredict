...
def real_time_forecasting02(sequence, diff_data, n_step,  k, maximum, minimum, MAX):

    #n_step, sigma = walk_forward_validation02(sequence, diff_data, n_steps, n_hold_out, maximum, minimum, MAX)

    history = reshape_inputs(diff_data, n_step)
    pred = [history[-1, 1:]]
    y_pred = train_and_predict(history, pred, k)
    return y_pred
...
def train_and_predict(data, pred, k):
...
def one_time_forecast(sequence, predictions, n_step, k, MAX):
    #start = time.monotonic()
    diff_data, maximum, minimum = pre_processing(sequence, MAX)

    y_pred = real_time_forecasting02(
        sequence, diff_data, n_step, k, maximum, minimum, MAX)
    y_pred = denormalize(y_pred, maximum, minimum)
...
def knn():
    ...
    for l in [5]:
        #f = open('./predictive_analysis/knn_changes_predictive_analysis.res', 'w') #w('+str(l)+')
        ...
        with open('./Datasets/test.data', 'r') as filehandle:
            ...
            for line in filecontents:
                ...
                for i in range(len(actual)+1):
                    ...
                    one_time_forecast(history, predictions, n_step, k, MAX)
                    if predictions[-1] < 0:
                        predictions[-1] = 0
