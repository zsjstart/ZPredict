...
def train_and_predict(data, pred, para):
...
def real_time_forecasting02(sequence, diff_data, n_step,  para, maximum, minimum, MAX):
    ...
    pred = [history[-1, 1:]]
    y_pred = train_and_predict(history, pred, para)
    return y_pred


def one_time_forecast_v2(sequence, predictions, n_step, para, MAX):
    #start = time.monotonic()
    diff_data, maximum, minimum = pre_processing(sequence, MAX)

    y_pred = real_time_forecasting02(
        sequence, diff_data, n_step, para, maximum, minimum, MAX)
    y_pred = denormalize(y_pred, maximum, minimum)
    prediction = (y_pred[0] + sequence[-1]) % MAX
...
def gp():
    ...
    for l in [5]:
        # f = open('./predictive_analysis/gp_changes_predictive_analysis.res', 'w') #w('+str(l)+')
        ...
        with open('./Datasets/test.data', 'r') as filehandle:
            ...
            for line in filecontents:
                ...
                for i in range(len(actual)+1):
                    ...
                    one_time_forecast_v2(
                        history, predictions, n_step, para, MAX)
                    if predictions[-1] < 0:
                        predictions[-1] = 0
                    end = time.monotonic()
