...
func RscanIpidVelocityTestIPV3(ip string, proto string, port uint16, domain string, fs, sl int) (int, []int, []int64) {
	...
	for i := 0; i< probes; i=i+2{
		...
		go func() {
			...
			//localIP := net.ParseIP("103.47.192.75").To4()
			ipid := 0
			...
			case "tcp":
				...
				riph,  _ = RscanRecvByRawConn(nil, extIP, localIP, port, localPort, 0, "tcp", "RA,R", "", 0, nil, timeout)
		...
		go func() {
			...
			//localIP := net.ParseIP("103.47.192.94").To4()
			ipid := 0
			var riph *ipv4.Header
			switch proto {
			case "icmp":
	...
	if float64(count_nega)/float64(len(ids)) > 0.30 { return -1, ids, times}
